...
The public API lets you drive your Tachyel CRM instance from your own
...
```
Authorization: Bearer tachyel_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** Tachyel CRM
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer tachyel_live_xxx"
```
...
Rather than polling, register an endpoint and Tachyel CRM will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; Tachyel CRM keeps only an encrypted copy.
...
Headers: `X-Tachyel-Event`, `X-Tachyel-Webhook-Id`, and `X-Tachyel-Signature`.

### Verifying the signature

`X-Tachyel-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages Tachyel CRM stores (inbox + API sends), not broadcast-only
